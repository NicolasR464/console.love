...
export default function SignUpModal({userID, onClose }: SignUpModalProps) {
  ...
  const userIDprop = userID
    ...
    const nextSlide = () => {setCurrentSlide(prevSlide => prevSlide + 1);console.log("JE CHANGE DE SLIDE")}
  ...
  const handleRemovePicture = (picture: String) => {
    ...
    //    SLIDE 3
       const submitSlide3 = (event: any) => {
        ...
        if(userData.attraction.length === 0) newErrors3.attraction = "This input is required";
       ...
       const submitSlide4 = (event: any) => {
        ...
        // Je valide si met input sont fill
        if(!userData.pictures || userData.pictures.length === 0) newErrors4.pictures = "This input is required";
        if(!userData.profilePicture) newErrors4.profilePicture = "A profile picture is required";
...
const submitSlide5 = (event: any) => {
  ...
  if(!userData.profileStatus) newErrors5.profileStatus = "This input is required";
  ...
  ) : null}            <div className="carousel w-full h-72">
            {currentSlide === 1 && 
                ...
                <input type="text" placeholder="First Name" className="input input-bordered input-info w-full my-2" 
                  name="firstName" value={userData.firstName} onChange={handleInputChange} />
                ...
                <input type="text" placeholder="Last Name" className="input input-bordered input-info w-full my-2" 
                  name="lastName" value={userData.lastName} onChange={handleInputChange} />
                ...
                {errors.lastName && <div className="text-red-600">{errors.lastName}</div>}
                <button type="submit" onClick={submitSlide1} className="btn btn-circle bg-blue-lover place-self-end">❯</button>
              </form>
  ...
  <div id="slide2" className="carousel-item relative w-full mx-2">
    <form className="flex flex-col mx-auto w-96">
      {errors2.age && <div className="text-red-600">{errors2.age}</div>}
      <input type="date"  value={userData.age} onChange={handleInputChange} name="age" className="input input-bordered input-info w-full my-2" />
      {errors2.languages && <div className="text-red-600">{errors2.languages}</div>}
      <div className='h-36 overflow-scroll' id="languagesSelect">
  {['Java', 'Go', 'C', 'C#', 'C++', 'Rust', 'JavaScript', 'Python'].map((language) => (
    <div key={language} className="m-1 flex">
      ...
      <div className="flex items-center">
        <label className='text-white self-center ml-2' htmlFor={language}>{language}</label>
      </div>
    </div>
  ))}
</div>
      <div className="absolute flex justify-between transform -translate-y-1/2 left-5 right-5 mt-60">
      <button type="submit" onClick={prevSlide} className="btn btn-circle bg-blue-lover ">❮</button> 
        <button type="submit" onClick={submitSlide2} className="btn btn-circle bg-blue-lover ">❯</button>
 ...
 {currentSlide === 3 && 
                ...
                <form className="flex flex-col mx-auto w-96">
                {errors3.sex && <div className="text-red-600">{errors3.sex}</div>}
                    <div className='flex m-2.5'>
                        <label className='label-text text-white font-bold text-xl'>Sex:</label> 
                        ...
                        />
                        <label className='label-text text-white m-1 ml-2.5	'>Female</label>
                        <input 
                        type="radio" 
                        name="sex" 
                        value="Female"
                        ...
                        className="radio radio-secondary" 
                        />
                        <label className='label-text text-white m-1 ml-2.5	'>Other</label>
                        <input 
                        type="radio" 
                        name="sex" 
                        value="Other"
                    ...
                    {errors3.attraction && <div className="text-red-600">{errors3.attraction}</div>}
                    <div className='flex m-2.5'>
                   
                        <label className='label-text text-white font-bold text-xl'>Attraction:</label> 
                        <label className='label-text text-white m-1'>Male</label> 
                                <input 
                                    type="checkbox" 
                                    name="attraction" 
                        ...
                        <div className="absolute flex justify-between transform -translate-y-1/2 left-5 right-5 mt-60">
      <button type="submit" onClick={prevSlide} className="btn btn-circle bg-blue-lover ">❮</button> 
        <button type="submit" onClick={submitSlide3} className="btn btn-circle bg-blue-lover ">❯</button>
...
/>
{errors4.pictures && <div className="text-red-600">{errors4.pictures}</div>}

<div className="flex text-white mt-2">
{userData.pictures.map((picture, index) => {
    let transformedUrl = picture.replace('/upload/', '/upload/w_100,h_100,c_fill,g_auto/'); 
    return (
...
{userData.pictures &&
<div className="text-white ">Select a profile picture among the uploaded images</div>}
{errors4.profilePicture && <div className="text-red-600">{errors4.profilePicture}</div>}
...
<div className='place-self-center text-white '>
  <b>Profile Pic:</b> 
    ...
    />
        ...
        <div className="absolute flex justify-between transform -translate-y-1/2 left-5 right-5 mt-60">
      <button type="submit" onClick={prevSlide} className="btn btn-circle bg-blue-lover ">❮</button> 
        <button type="submit" onClick={submitSlide4} className="btn btn-circle bg-blue-lover ">❯</button>
                ...
                <button className={`btn h-36 ${userData.profileStatus === 'Merge' ? 'btn-primary' : 'btn-active'}`} 
                    onClick={(e) => {e.preventDefault(); setUserData(prev => ({ ...prev, profileStatus: 'Merge' }))}}>
                    <div>
                      <Image style={{ margin: "auto", }} src={MergeImg} width={75} height={75} alt=""></Image>
                    <div className='mt-5'>  <span className='text-pink-lover'>Merge</span> <span className='text-blue-lover'>one night</span>
                    </div></div>
                  </button>
                  <div className="divider divider-horizontal text-white font-bold">OR</div>
                  <button className={`btn h-36 ${userData.profileStatus === 'Const' ? 'btn-primary' : 'btn-active'}`} 
                    onClick={(e) => {e.preventDefault(); setUserData(prev => ({ ...prev, profileStatus: 'Const' }))}}>
                    <div>
                      <Image style={{ margin: "auto", }} src={ConstImg} width={75} height={75} alt=""></Image>
                      <div className='mt-5'> <span className='text-pink-lover'>Const</span> <span className='text-blue-lover'>for life</span>
                      </div>  </div>
              
                  </button>
                  {errors5.profileStatus && <div className="text-red-600">{errors5.profileStatus}</div>}
                  </div>
      ...
      <button type="submit" onClick={prevSlide} className="btn btn-circle bg-blue-lover ">❮</button> 
      {currentPath === "/myprofile" ? (
    <button className='btn bg-pink-lover ' onClick={submitSlide5}>Edit</button>
  ) : null}  
    {currentPath === "/complete_profile" ? (
    <button className='btn bg-pink-lover ' onClick={submitSlide5}>Start the adventure</button>
  ) : null}          
            ...
            {currentPath === "/myprofile" ? (
    <button className='btn btn-error mx-auto' onClick={onClose}>Close</button>
  ) : null}      
           </div>
        </div>
    </div>
